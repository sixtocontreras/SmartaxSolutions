...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class UnidadesMedida
    {
        ...
        public DataTable GetAllUnidadMedidas()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtUnidadMedida";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_unidad_medida", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("descripcion_medida");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idunidad_medida"] = Convert.ToInt32(TheDataReaderPostgreSQL["idunidad_medida"].ToString().Trim());
                            Fila["unidad_medida"] = TheDataReaderPostgreSQL["unidad_medida"].ToString().Trim();
                            Fila["descripcion_medida"] = TheDataReaderPostgreSQL["descripcion_medida"].ToString().Trim();
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_unidad_medida";
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idunidad_medida"] = Convert.ToInt32(TheDataReaderSQLServer["idunidad_medida"].ToString().Trim());
                            Fila["unidad_medida"] = TheDataReaderSQLServer["unidad_medida"].ToString().Trim();
                            Fila["descripcion_medida"] = TheDataReaderSQLServer["descripcion_medida"].ToString().Trim();
        ...
        public DataTable GetUnidadMedidas()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtUnidadMedida";
            try
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        ...
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idunidad_medida"] = TheDataReaderSQLServer["idunidad_medida"].ToString().Trim();
                            Fila["descripcion_medida"] = TheDataReaderSQLServer["descripcion_medida"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
        ...
        public bool AddUpUnidadMedida(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_unidad_medida", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idunidad_medida", IdUnidadMedida);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_unidad_medida", Fila["unidad_medida"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_descripcion_medida", Fila["descripcion_medida"].ToString().Trim().ToUpper());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer = new SqlCommand();
                    TheCommandSQLServer.CommandText = "sp_web_crud_unidad_medida";
                    TheCommandSQLServer.CommandType = CommandType.StoredProcedure;
                    TheCommandSQLServer.Connection = (System.Data.SqlClient.SqlConnection)myConnectionDb;
                    //se limpian los parámetros
                    TheCommandSQLServer.Parameters.Clear();

                    UnidadMedida = Fila["descripcion_medida"].ToString().Trim().ToUpper();
                    ...
                    if (Valor_Retornado >= 1)
                    {
                        if (TipoProceso == 1)
                        {
                            ...
                            _MsgError = "La unidad de medida [" + UnidadMedida + "] ha sido registrado exitosamente.";
                        }
                        else if (TipoProceso == 2)
                        {
                            _MsgError = "La unidad de medida [" + UnidadMedida + "] han sido editados exitosamente.";
                        }
                        else if (TipoProceso == 3)
                        {
                            _MsgError = "La unidad de medida [" + UnidadMedida + "] ah sido eliminado del sistema.";
                        }
