...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class TipoNivel
    {
        ...
        public object IdTipoNivel { get; set; }
        ...
        public DataTable GetAllTipoNivel()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtTipoNivel";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_tipo_nivel", (PgSqlConnection)myConnectionDb);
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idtipo_nivel"] = Convert.ToInt32(TheDataReaderPostgreSQL["idtipo_nivel"].ToString().Trim());
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_tipo_nivel]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetTipoNivel()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TablaDatos.Columns.Add("idtipo_nivel");
                    TablaDatos.Columns.Add("tipo_nivel");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        ...
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_nivel"] = TheDataReaderPostgreSQL["idtipo_nivel"].ToString().Trim();
                            Fila["tipo_nivel"] = TheDataReaderPostgreSQL["tipo_nivel"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
        ...
        public bool AddUpTipoNivel(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_tipo_nivel", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idtipo_nivel", IdTipoNivel);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_nivel", Fila["tipo_nivel"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idcliente", Fila["id_cliente"].ToString().Trim().Length > 0 ? Fila["id_cliente"].ToString().Trim() : null);
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el Tipo de nivel. Motivo: " + ex.Message.ToString().Trim();
