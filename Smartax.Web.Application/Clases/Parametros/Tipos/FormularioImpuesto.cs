...
{
    public class FormularioImpuesto
    {
        ...
        public DataTable GetAllFormularioImpuesto()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtFormularioImpuesto";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_formulario_impuesto", (PgSqlConnection)myConnectionDb);
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_formulario_impuesto";
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_formulario_impuesto]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetFormularioImpuesto()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtFormularioImpuesto";
            try
        ...
        public bool AddUpFormularioImpuesto(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_formulario_impuesto", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idformulario_impuesto", IdFormularioImpuesto);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_descripcion", Fila["descripcion_formulario"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idestado", Fila["id_estado"].ToString().Trim());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idusuario_up", IdUsuario);
