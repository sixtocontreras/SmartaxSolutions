...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class Banco
    {
        ...
        public DataTable GetAllBancos()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtBancos";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_bancos", (PgSqlConnection)myConnectionDb);
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["id_banco"] = Convert.ToInt32(TheDataReaderPostgreSQL["id_banco"].ToString().Trim());
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_bancos]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetBancos()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TablaDatos.Columns.Add("id_banco");
                    TablaDatos.Columns.Add("nombre_banco");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        ...
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["id_banco"] = TheDataReaderPostgreSQL["id_banco"].ToString().Trim();
                            Fila["nombre_banco"] = TheDataReaderPostgreSQL["nombre_banco"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
        ...
        public bool AddUpBanco(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_banco", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idbanco", IdBanco);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_descripcion", Fila["nombre_banco"].ToString().Trim().ToUpper());
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el banco. Motivo: " + ex.Message.ToString().Trim();
