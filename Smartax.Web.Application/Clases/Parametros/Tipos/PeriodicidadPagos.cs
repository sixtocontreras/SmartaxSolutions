...
{
    public class PeriodicidadPagos
    {
        ...
        public DataTable GetAllPeriodicidadPagos()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_periodicidad_pago", (PgSqlConnection)myConnectionDb);
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["id_periodicidad"] = Convert.ToInt32(TheDataReaderPostgreSQL["id_periodicidad"].ToString().Trim());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_periodicidad_pago";
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_periodicidad_pago]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetPeriodicidadPagos()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtPeriodicidadPagos";
            try
        ...
        public DataTable GetPeriodicidadMunicipio()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtPeriodicidadPagos";
            try
        ...
        public DataTable GetPeriodicidadImpuesto()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtPeriodicidadImpuesto";
            try
            {
                #region DEFINICION OBJETO DE CONEXION A LA DB.
                StringBuilder sSQL = new StringBuilder();
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    #region OBTENER DATOS DE LA DB DE POSTGRESQL
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_periodicidad_impuesto", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();
        ...
        public bool AddUpPeriodicidadPagos(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_periodicidad_pago", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idperiodicidad", IdPeriodicidad);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_descripcion", Fila["descripcion_periodicidad"].ToString().Trim().ToUpper());
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar la Periodicidad de pagos. Motivo: " + ex.Message.ToString().Trim();
