...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class TiposMoneda
    {
        ...
        public DataTable GetAllTipoMoneda()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtTipoMoneda";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_tipo_moneda", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("tipo_moneda");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idtipo_moneda"] = Convert.ToInt32(TheDataReaderPostgreSQL["idtipo_moneda"].ToString().Trim());
                            Fila["codigo_moneda"] = TheDataReaderPostgreSQL["codigo_moneda"].ToString().Trim();
                            Fila["tipo_moneda"] = TheDataReaderPostgreSQL["tipo_moneda"].ToString().Trim();
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_tipo_moneda";
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_moneda"] = Convert.ToInt32(TheDataReaderSQLServer["idtipo_moneda"].ToString().Trim());
                            Fila["codigo_moneda"] = TheDataReaderSQLServer["codigo_moneda"].ToString().Trim();
                            Fila["tipo_moneda"] = TheDataReaderSQLServer["tipo_moneda"].ToString().Trim();
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_tipo_moneda]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetTipoMoneda()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TablaDatos.Columns.Add("idtipo_moneda");
                    TablaDatos.Columns.Add("codigo_moneda");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        ...
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_moneda"] = TheDataReaderPostgreSQL["idtipo_moneda"].ToString().Trim();
                            Fila["codigo_moneda"] = TheDataReaderPostgreSQL["codigo_moneda"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        ...
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_moneda"] = TheDataReaderSQLServer["idtipo_moneda"].ToString().Trim();
                            Fila["tipo_moneda"] = TheDataReaderSQLServer["tipo_moneda"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
        ...
        public bool AddUpTipoMoneda(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_tipo_moneda", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idtipo_cargo", IdTipoMoneda);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_cod_moneda", Fila["codigo_moneda"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_cargo", Fila["tipo_moneda"].ToString().Trim().ToUpper());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer = new SqlCommand();
                    TheCommandSQLServer.CommandText = "sp_web_crud_tipo_moneda";
                    TheCommandSQLServer.CommandType = CommandType.StoredProcedure;
                    TheCommandSQLServer.Connection = (System.Data.SqlClient.SqlConnection)myConnectionDb;
                    //se limpian los parámetros
                    TheCommandSQLServer.Parameters.Clear();

                    TipoMoneda = Fila["tipo_moneda"].ToString().Trim().ToUpper();
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idtipo_cargo", IdEstado);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_tipo_cargo", Fila["descripcion_estado"].ToString().Trim());
                    ...
                    if (Valor_Retornado >= 1)
                    {
                        if (TipoProceso == 1)
                        {
                            ...
                            _MsgError = "El tipo de moneda [" + TipoMoneda + "] ha sido registrado exitosamente.";
                        }
                        else if (TipoProceso == 2)
                        {
                            _MsgError = "El tipo de moneda [" + TipoMoneda + "] han sido editados exitosamente.";
                        }
                        else if (TipoProceso == 3)
                        {
                            _MsgError = "El tipo de moneda [" + TipoMoneda + "] ah sido eliminado del sistema.";
                        }
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el tipo de moneda. Motivo: " + ex.Message.ToString().Trim();
