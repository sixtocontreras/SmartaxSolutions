...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class TiposTarifa
    {
        ...
        public object Interfaz { get; set; }
        public int TipoConsulta { get; set; }
        public int TipoProceso { get; set; }
        #endregion

        public DataTable GetAllTipoTarifa()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtTipoTarifa";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_tipo_tarifa", (PgSqlConnection)myConnectionDb);
                    ...
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_interfaz", Interfaz);
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idtipo_tarifa"] = Convert.ToInt32(TheDataReaderPostgreSQL["idtipo_tarifa"].ToString().Trim());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_tipo_tarifa";
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_tarifa"] = Convert.ToInt32(TheDataReaderSQLServer["idtipo_tarifa"].ToString().Trim());
                            Fila["descripcion_tarifa"] = TheDataReaderSQLServer["descripcion_tarifa"].ToString().Trim();
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_tipo_tarifa]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetTipoTarifa()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtTipoTarifa";
            try
        ...
        public bool AddUpTipoTarifa(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_tipo_tarifa", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idtipo_tarifa", IdTipoTarifa);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_descripcion", Fila["descripcion_tarifa"].ToString().Trim().ToUpper());
