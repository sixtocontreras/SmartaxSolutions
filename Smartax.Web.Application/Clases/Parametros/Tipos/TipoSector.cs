...
namespace Smartax.Web.Application.Clases.Parametros.Tipos
{
    public class TipoSector
    {
        ...
        public DataTable GetAllNombreSector()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtNombreSector";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_tipo_sector", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("tipo_sector");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idtipo_sector"] = Convert.ToInt32(TheDataReaderPostgreSQL["idtipo_sector"].ToString().Trim());
                            Fila["tipo_sector"] = TheDataReaderPostgreSQL["tipo_sector"].ToString().Trim();
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_tipo_sector";
                    ...
                    if (TheDataReaderSQLServer != null)
                    {
                        while (TheDataReaderSQLServer.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["idtipo_sector"] = Convert.ToInt32(TheDataReaderSQLServer["idtipo_sector"].ToString().Trim());
                            Fila["tipo_sector"] = TheDataReaderSQLServer["tipo_sector"].ToString().Trim();
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_tipo_sector]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetTipoSector()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtTipoSector";
            try
            {
                #region DEFINICION OBJETO DE CONEXION A LA DB.
                StringBuilder sSQL = new StringBuilder();
        ...
        public bool AddUpNombreSector(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_tipo_sector", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idtipo_sector", IdTipoSector);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_sector", Fila["tipo_sector"].ToString().Trim().ToUpper());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer = new SqlCommand();
                    TheCommandSQLServer.CommandText = "sp_web_crud_tipo_sector";
                    TheCommandSQLServer.CommandType = CommandType.StoredProcedure;
                    TheCommandSQLServer.Connection = (System.Data.SqlClient.SqlConnection)myConnectionDb;
                    //se limpian los parámetros
                    TheCommandSQLServer.Parameters.Clear();

                    NombreSector = Fila["tipo_sector"].ToString().Trim().ToUpper();
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idtipo_sector", IdEstado);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_tipo_sector", Fila["descripcion_estado"].ToString().Trim());
                    ...
                    if (Valor_Retornado >= 1)
                    {
                        if (TipoProceso == 1)
                        {
                            ...
                            _MsgError = "El tipo de firma [" + NombreSector + "] ha sido registrado exitosamente.";
                        }
                        else if (TipoProceso == 2)
                        {
                            _MsgError = "El tipo de firma [" + NombreSector + "] han sido editados exitosamente.";
                        }
                        else if (TipoProceso == 3)
                        {
                            _MsgError = "El tipo de firma [" + NombreSector + "] ah sido eliminado del sistema.";
                        }
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el Tipo de firma. Motivo: " + ex.Message.ToString().Trim();
