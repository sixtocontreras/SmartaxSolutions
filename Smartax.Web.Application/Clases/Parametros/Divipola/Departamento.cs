...
namespace Smartax.Web.Application.Clases.Parametros.Divipola
{
    public class Departamento
    {
        ...
        public DataTable GetAllDptos()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtDepartamentos";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_dptos", (PgSqlConnection)myConnectionDb);
                    ...
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idpais", IdPais);
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["id_pais"] = Convert.ToInt32(TheDataReaderPostgreSQL["id_pais"].ToString().Trim());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer.CommandText = "sp_web_get_dptos";
        ...
        public DataTable GetDptos()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtDptos";
            try
            {
                #region DEFINICION OBJETO DE CONEXION A LA DB.
                StringBuilder sSQL = new StringBuilder();
        ...
        public bool AddUpDpto(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_dpto", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_iddpto", IdDpto);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idpais", Fila["id_pais"].ToString().Trim());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_codigo_dane", Fila["codigo_dane"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_nombre_dpto", Fila["nombre_departamento"].ToString().Trim().ToUpper());
                ...
                else if (myConnectionDb is SqlConnection)
                {
                    ...
                    TheCommandSQLServer = new SqlCommand();
                    TheCommandSQLServer.CommandText = "sp_web_crud_dpto";
                    TheCommandSQLServer.CommandType = CommandType.StoredProcedure;
                    TheCommandSQLServer.Connection = (System.Data.SqlClient.SqlConnection)myConnectionDb;
                    //se limpian los parámetros
                    TheCommandSQLServer.Parameters.Clear();

                    NombreDpto = Fila["nombre_departamento"].ToString().Trim().ToUpper();
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_iddpto", IdPais);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idpais", Fila["id_pais"].ToString().Trim());
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_codigo_dane", Fila["codigo_dane"].ToString().Trim().ToUpper());
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_nombre_dpto", _NombreDpto);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idestado", Fila["id_estado"]);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idusuario_add", IdUsuarioAdd);
                    TheCommandSQLServer.Parameters.AddWithValue("@p_in_idusuario_up", IdUsuarioUp);
                    ...
                    if (Valor_Retornado >= 1)
                    {
                        if (TipoProceso == 1)
                        {
                            ...
                            _MsgError = "El departamento [" + _NombreDpto + "] ha sido registrado exitosamente.";
                        }
                        else if (TipoProceso == 2)
                        {
                            _MsgError = "El departamento [" + _NombreDpto + "] han sido editados exitosamente.";
                        }
                        else if (TipoProceso == 3)
                        {
                            _MsgError = "El departamento [" + _NombreDpto + "] ah sido eliminado del sistema.";
                        }
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el departamento. Motivo: " + ex.Message.ToString().Trim();
