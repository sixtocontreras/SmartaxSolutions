...
namespace Smartax.Web.Application.Clases.Parametros.Provision
{
    public class EjecutarProv
    {
        ...
        internal bool ExecProv(ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                {
                    #region EJECUTAR SP EN LA DB POSTGRESQL
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_task_provision_ica", (PgSqlConnection)myConnectionDb);
            ...
            {
                retValor = false;
                _MsgError = "Error al procesar la provisión. Motivo: " + ex.Message.ToString().Trim();
