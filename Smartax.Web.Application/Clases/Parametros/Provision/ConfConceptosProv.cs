...
{
    public class ConfConceptosProv
    {
        ...
        public object IdConfConceptosProvIca { get; set; }
        public object IdCliente { get; set; }
        public int Vigencia { get; set; }
        public object IdConceptosProvIca { get; set; }
        public string CtaDebito { get; set; }
        public string CtaCredito { get; set; }
        public string CentroCosto { get; set; }
        public object Renglon { get; set; }
        ...
        internal DataTable GetAllConfConceptosProv()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtConfConceptosProv";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_conf_conceptos_prov_ica", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("id_conf_conceptos_prov_ica", typeof(Int32));
                    TablaDatos.Columns.Add("id_cliente", typeof(Int32));
                    TablaDatos.Columns.Add("nombre_cliente");
                    TablaDatos.Columns.Add("vigencia", typeof(Int32));
                    TablaDatos.Columns.Add("id_conceptos_prov_ica", typeof(Int32));
                    TablaDatos.Columns.Add("desc_concepto");
                    TablaDatos.Columns.Add("id_cta_debito");
                    TablaDatos.Columns.Add("cta_debito");
                    TablaDatos.Columns.Add("id_cta_credito");
                    TablaDatos.Columns.Add("cta_credito");
                    TablaDatos.Columns.Add("centro_costo");
                    TablaDatos.Columns.Add("renglon", typeof(Int32));
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["id_conf_conceptos_prov_ica"] = Convert.ToInt32(TheDataReaderPostgreSQL["id_conf_conceptos_prov_ica"].ToString().Trim());
                            ...
                            Fila["vigencia"] = Convert.ToInt32(TheDataReaderPostgreSQL["vigencia"].ToString().Trim());
                            Fila["id_conceptos_prov_ica"] = Convert.ToInt32(TheDataReaderPostgreSQL["id_conceptos_prov_ica"].ToString().Trim());
                            Fila["desc_concepto"] = TheDataReaderPostgreSQL["desc_concepto"].ToString().Trim();
                            Fila["cta_debito"] = TheDataReaderPostgreSQL["cta_debito"].ToString().Trim();
                            Fila["cta_credito"] = TheDataReaderPostgreSQL["cta_credito"].ToString().Trim();
                            Fila["centro_costo"] = TheDataReaderPostgreSQL["centro_costo"].ToString().Trim();
                            Fila["renglon"] = Convert.ToInt32(TheDataReaderPostgreSQL["renglon"].ToString().Trim());
        ...
        internal bool AddUpConceptoProv(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_conf_conceptos_prov_ica", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_id_conf_conceptos_prov_ica", _IdRegistro);
                    ...
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_id_cod_concepto", IdConceptosProvIca);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_id_cta_debito", CtaDebito);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_id_cta_crebito", CtaCredito);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_centro_costo", CentroCosto.ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_renglon", Renglon);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idestado", IdEstado);
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar el Estado. Motivo: " + ex.Message.ToString().Trim();
