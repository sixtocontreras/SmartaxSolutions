...
namespace Smartax.Web.Application.Clases.Seguridad
{
    public class SistemaRol
    {
        ...
        public object IdComercio { get; set; }
        ...
        public DataTable GetAllRoles()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_sistema_rol", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("rol_sistema", typeof(Boolean));
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            if (TheDataReaderPostgreSQL["rol_sistema"].ToString().Trim().Equals("1"))
                            {
                                Fila["rol_sistema"] = true;
                            }
                            else
                            {
                                Fila["rol_sistema"] = false;
                            }
            ...
            {
                _log.Error("Error al obtener los datos de la Tabla [tbl_sistema_rol]. Motivo: " + ex.Message);
            }
            finally
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        ...
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            DataRow Fila = null;
                            Fila = TablaDatos.NewRow();
                            Fila["id_rol"] = TheDataReaderPostgreSQL["id_rol"].ToString().Trim();
                            Fila["nombre_rol"] = TheDataReaderPostgreSQL["nombre_rol"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
        ...
        public bool AddUpRoles(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_sistema_rol", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idrol", IdRol);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_nombre_rol", Fila["nombre_rol"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_rol_sistema", Fila["rol_sistema"]);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idcliente", IdCliente);
        ...
        public bool SetProcesoSistemaRol(ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_set_proceso_sistema_rol", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idrol", IdRol);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_nombre_rol", NombreRol);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_proceso", TipoProceso);
                    PgSqlParameter _IdRegRetorno = new PgSqlParameter("@p_out_id_registro", SqlDbType.Int);
                    PgSqlParameter _CodRptaRetorno = new PgSqlParameter("@p_out_cod_rpta", SqlDbType.VarChar);
                    PgSqlParameter _MsgRptaRetorno = new PgSqlParameter("@p_out_msg_rpta", SqlDbType.VarChar);
