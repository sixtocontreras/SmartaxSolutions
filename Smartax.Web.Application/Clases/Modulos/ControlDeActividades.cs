...
namespace Smartax.Web.Application.Clases.Modulos
{
    public class ControlDeActividades
    {
        ...
        public object IdControlActivRol { get; set; }
        public object IdControlActivUser { get; set; }
        ...
        public DataTable GetMisActividades()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtMisActividades";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_mis_actividades", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("fecha_cierre");
                    TablaDatos.Columns.Add("id_estado", typeof(Int32));
                    TablaDatos.Columns.Add("estado_actividad");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idcontrol_activ_rol"] = Int32.Parse(TheDataReaderPostgreSQL["idcontrol_activ_rol"].ToString().Trim());
                            ...
                            int _IdEstado = Int32.Parse(TheDataReaderPostgreSQL["id_estado"].ToString().Trim());
                            //--VALIDAR EL ESTADO DE LA ACTIVIDAD
                            if (_IdEstado == 1)
                            {
                                Fila["porc_cumplimiento"] = 0;
                            }
                            else
                            {
                                Fila["porc_cumplimiento"] = Double.Parse(TheDataReaderPostgreSQL["porc_cumplimiento"].ToString().Trim());
                            }
                            Fila["id_estado"] = _IdEstado;
                            Fila["estado_actividad"] = TheDataReaderPostgreSQL["estado_actividad"].ToString().Trim();
                            Fila["fecha_cierre"] = TheDataReaderPostgreSQL["fecha_cierre_actividad"].ToString().Trim();
            ...
            {
                _log.Error("Error al obtener datos del sp [sp_web_get_mis_actividades]. Motivo: " + ex.Message);
            }
            finally
            {
        ...
        public DataTable GetEstadisticaImpuestos()
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    #region OBTENER DATOS DE LA DB DE POSTGRESQL
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_estadistica_impuestos", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();
                    ...
                    TablaDatos.Columns.Add("cantidad_liquidaciones", typeof(Int32));
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["cantidad_liquidaciones"] = Int32.Parse(TheDataReaderPostgreSQL["cantidad_liquidaciones"].ToString().Trim());
                            TablaDatos.Rows.Add(Fila);
                        }
                    }
                    #endregion
        ...
        public bool AddUpMisActividades(ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_mis_actividades", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idcontrol_activ_user", IdControlActivUser);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idcontrol_activ_rol", IdControlActivRol);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idrol", IdRol);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idusuario", IdUsuario);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idestado", IdEstado);
            ...
            {
                retValor = false;
                _MsgError = "Error al gectionar la actividad. Motivo: " + ex.Message.ToString().Trim();
