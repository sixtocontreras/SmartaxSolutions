...
namespace Smartax.Web.Application.Clases.Administracion
{
    public class EjecucionXLote
    {
        ...
        public DataTable GetAllEjecucionXLote()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtEjecucionXLote";
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_get_ejecucion_lote", (PgSqlConnection)myConnectionDb);
                    ...
                    TablaDatos.Columns.Add("idejecucion_lote", typeof(Int32));
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["idejecucion_lote"] = Convert.ToInt32(TheDataReaderPostgreSQL["idejecucion_lote"].ToString().Trim());
        ...
        public DataTable GetEjecucionXLote()
        {
            DataTable TablaDatos = new DataTable();
            TablaDatos.TableName = "DtEjecucionXLote";
            try
        ...
        public bool AddUpEjecucionXLote(DataRow Fila, ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_ejecucion_lote", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idejecucion_lote", IdEjecucionLote);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idcliente", Fila["id_cliente"].ToString().Trim());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_nombre_proceso", Fila["nombre_proceso"].ToString().Trim().ToUpper());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idestado", Fila["id_estado"].ToString().Trim());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idempresa", Fila["id_empresa"].ToString().Trim());
            ...
            {
                retValor = false;
                _MsgError = "Error al registrar la ejecucion x lote. Motivo: " + ex.Message.ToString().Trim();
