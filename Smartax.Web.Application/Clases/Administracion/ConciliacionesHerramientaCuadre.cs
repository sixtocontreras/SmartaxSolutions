...
namespace Smartax.Web.Application.Clases.Administracion
{
    public class ConciliacionesHerramientaCuadre
    {
        ...
        public object TipoPeriodicidad { get; set; }
        ...
        public DataTable GetArchivosDavibox(ref string _MsgError)
        {
            ...
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    ...
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_proceso", TipoArchivo);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_idestado", IdEstado);
                    TheDataReaderPostgreSQL = TheCommandPostgreSQL.ExecuteReader();
                    ...
                    TablaDatos.Columns.Add("uuid_folder");
                    ...
                    if (TheDataReaderPostgreSQL != null)
                    {
                        while (TheDataReaderPostgreSQL.Read())
                        {
                            ...
                            Fila["tipo_periodicidad"] = TheDataReaderPostgreSQL["tipo_periodicidad"].ToString().Trim();
                            Fila["periodicidad"] = TheDataReaderPostgreSQL["periodicidad"].ToString().Trim();
                            Fila["uuid_folder"] = TheDataReaderPostgreSQL["uuid_folder"].ToString().Trim();
                            TablaDatos.Rows.Add(Fila);
                            break;
                        }
                        _MsgError = "";
        ...
        public bool UpUuidFilesDavibox(ref int _IdRegistro, ref string _MsgError)
        {
            bool retValor = false;
            try
            {
                ...
                if (myConnectionDb is PgSqlConnection)
                {
                    #region REGISTRAR DATO CON EL SP EN LA DB POSTGRESQL
                    IDbTransaction Transac = myConnectionDb.BeginTransaction();
                    TheCommandPostgreSQL = new PgSqlCommand("sp_web_crud_archivos_davibox", (PgSqlConnection)myConnectionDb);
                    TheCommandPostgreSQL.CommandType = CommandType.StoredProcedure;
                    //Limpiar parametros
                    TheCommandPostgreSQL.Parameters.Clear();

                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_archivo", TipoArchivo);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_uuid_folder", UuId.ToString().Trim());
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_anio_gravable", AnioGravable);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_periodicidad", TipoPeriodicidad);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_periodicidad", Periodicidad);
                    TheCommandPostgreSQL.Parameters.AddWithValue("@p_in_tipo_proceso", TipoProceso);
                    PgSqlParameter _IdRegRetorno = new PgSqlParameter("@p_out_id_registro", SqlDbType.Int);
                    PgSqlParameter _CodRptaRetorno = new PgSqlParameter("@p_out_cod_rpta", SqlDbType.VarChar);
                    PgSqlParameter _MsgRptaRetorno = new PgSqlParameter("@p_out_msg_rpta", SqlDbType.VarChar);
            ...
            {
                retValor = false;
                _MsgError = "Error al actualizar el Uuid de los archivos del davibox. Motivo: " + ex.Message.ToString().Trim();
                _log.Error(_MsgError.ToString().Trim());
            }
            finally
            {
