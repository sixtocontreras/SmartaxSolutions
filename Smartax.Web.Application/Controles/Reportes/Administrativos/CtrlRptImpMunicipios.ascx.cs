    ...
    public partial class CtrlRptImpMunicipios : System.Web.UI.UserControl
    {
        ...
        protected void BtnGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string _MsgError = "";
                dtDatos = ObjReporte.GetRptImpMunicipio(ref _MsgError);
        ...
        protected void ExportarDatosExcel(DataTable DtDatosExportar)
        {
            try
            {
                if (DtDatosExportar.Rows.Count > 0)
                {
                    ...
                    string cNombreFileExcel = "RptImpMunicipios_" + _DiaActual + "_" + _MesActual + ".xlsx";
                    ...
                    string strNombreEncabezadoReporte = "REPORTE DE IMPUESTOS MUNICIPIOS - ESTADO [" + strEstado + "]";
                    ...
                    for (int ncol = 0; ncol < DtDatosExportar.Columns.Count; ncol++)
                    {
                        ...
                        for (int nrow = 0; nrow < DtDatosExportar.Rows.Count; nrow++)
                        {
                            ...
                            if (ncol == 7)
                            {
                                sheet.Cells[Row, ContadorCol].NumberFormat = FormatoValor;
                                sheet.Cells[Row, ContadorCol].Value = Int32.Parse(DtDatosExportar.Rows[nrow][ncol].ToString().Trim());
                    ...
                    if (Contador > 0)
                    {
                        ...
                        book.Worksheets["Sheet1"].Name = "IMPUESTOS_MUNICIPIO";
                        book.Worksheets["IMPUESTOS_MUNICIPIO"].UsedRange.Autofit();
                        Response.Clear();
