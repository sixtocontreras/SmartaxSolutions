...
{
    public partial class CtrlRptOtrasConfig : System.Web.UI.UserControl
    {
        ...
        protected void BtnGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string _MsgError = "";
                dtDatos = ObjReporte.GetRptOtrasConfiguraciones(ref _MsgError);
        ...
        protected void ExportarDatosExcel(DataTable DtDatosExportar)
        {
            try
            {
                if (DtDatosExportar.Rows.Count > 0)
                {
                    ...
                    string cNombreFileExcel = "RptOtrasConfiguraciones_" + _DiaActual + "_" + _MesActual + ".xlsx";
                    ...
                    string strNombreEncabezadoReporte = "REPORTE DE OTRAS CONFIGURACIONES A MUNICIPIOS - ESTADO [" + strEstado + "]";
                    ...
                    for (int ncol = 0; ncol < DtDatosExportar.Columns.Count; ncol++)
                    {
                        ...
                        for (int nrow = 0; nrow < DtDatosExportar.Rows.Count; nrow++)
                        {
                            ...
                            if (ncol == 14 || ncol == 15 || ncol == 16 || ncol == 17)
                            {
                    ...
                    if (Contador > 0)
                    {
                        ...
                        book.Worksheets["Sheet1"].Name = "OTRAS_CONFIGURACIONES";
                        book.Worksheets["OTRAS_CONFIGURACIONES"].UsedRange.Autofit();
                        Response.Clear();
