...
{
    public partial class CtrlConsultaLiquidacionImpuestos : System.Web.UI.UserControl
    {
        ...
        protected void LstEstados()
        {
            try
            {
                ObjEstado.TipoConsulta = 2;
                ObjEstado.TipoEstado = "CONSULTA_LIQUIDACIONES";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.Page.Title = this.Page.Title + "Consulta Liquidación de Impuestos";
                ...
                else if (e.CommandName == "BtnAnularLiquidacion")
                {
                    ...
                    if (_IdEstado != 5)
                    {
                        ...
                        if (ObjLiqImpuesto.AddUpLiquidacionImpuestoIca(ref _IdLiquidImpuesto, ref _MsgError))
                        {
                            ...
                            ObjAuditoria.ModuloApp = "ANULAR_LIQUIDACIONES";
        ...
        private void GetConsultarLiquidacion()
        {
            ...
            try
            {
                //--AQUI VALIDAMOS QUE TENGA SELECCIONADO EL TIPO DE IMPUESTO Y AÑO GRAVABLE
                if (this.CmbTipoImpuesto.SelectedValue.ToString().Trim().Length > 0)
                {
                    if (this.TxtAnioGravable.Text.ToString().Trim().Length > 0)
                    {
                        ...
                        ObjLiqImpuesto.IdUsuario = this.CmbUsuarios.SelectedValue.ToString().Trim().Length > 0 ? this.CmbUsuarios.SelectedValue.ToString().Trim() : null;
                        ObjLiqImpuesto.IdEstado = this.CmbEstado.SelectedValue.ToString().Trim().Length > 0 ? this.CmbEstado.SelectedValue.ToString().Trim() : null;
                        ObjLiqImpuesto.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                    ...
                    else
                    {
                        ...
                        RadWindow Ventana = new RadWindow();
                        Ventana.Modal = true;
                        string _MsgError = "Señor usuario, debe indicar un año gravable !";
                        Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                        Ventana.ID = "RadWindow" + ObjUtils.GetRandom();
                        Ventana.VisibleOnPageLoad = true;
                        Ventana.Visible = true;
