...
{
    public partial class FrmMonitoreoActividades : System.Web.UI.Page
    {
        ...
        private DataTable GetDatos()
        {
            ...
            try
            {
                ...
                dtDatos = ObjActividad.GetMonitoreoActividades();
                //--
                if (dtDatos != null)
                {
                    ...
                    {
                        //--DATOS
                        this.ViewState["DtMonitoreoActividades"] = dtDatos;
                        foreach (DataRow rowItem in dtDatos.Rows)
                        {
                            int _IdControlActividad = Int32.Parse(rowItem["idcontrol_actividad"].ToString().Trim());

                            DataRow[] dataRows = dtDatos.Select("idcontrol_actividad = " + _IdControlActividad);
                            if (dataRows.Length == 1)
                            {
                                double _Cantidad = Double.Parse(rowItem["cantidad"].ToString().Trim());
                                double _CantidadSi = Double.Parse(rowItem["cantidad_si"].ToString().Trim());
                                double _CantidadNo = Double.Parse(rowItem["cantidad_no"].ToString().Trim());
                                double _Total = Double.Parse(rowItem["total"].ToString().Trim());
                                double _PorcentajeSi = _CantidadSi > 0 ? ((_CantidadSi / _Total) * 100) : 0;
                                double _PorcentajeNo = _CantidadNo > 0 ? ((_CantidadNo / _Total) * 100) : 0;

                                dataRows[0]["porcentaje_si"] = Math.Round(_PorcentajeSi, 2);
                                dataRows[0]["porcentaje_no"] = Math.Round(_PorcentajeNo, 2); //--round(Math.Abs(_PorcentajeNo));
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                //--DATOS
                this.ViewState["DtMonitoreoActividades"] = null;
                //--
        ...
        protected void RadGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            ...
            {
                DataTable dtDatos = new DataTable();
                dtDatos = (DataTable)this.ViewState["DtMonitoreoActividades"];
                //--
                if (dtDatos != null)
                {
                    if (dtDatos.Rows.Count > 0)
                    {
                        ...
                        this.RadGrid1.DataMember = "DtMonitoreoActividades";
                    }
                    else
                    {
                        this.RadGrid1.DataSource = dtDatos;
        ...
        protected void RbtTipoConsulta_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                if (this.RbtTipoConsulta.SelectedValue.ToString().Trim().Equals("1"))
                {
