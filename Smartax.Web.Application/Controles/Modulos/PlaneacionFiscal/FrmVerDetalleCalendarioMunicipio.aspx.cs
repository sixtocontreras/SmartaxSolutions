...
{
    public partial class FrmVerDetalleCalendarioMunicipio : System.Web.UI.Page
    {
        ...
        public DataSet GetDatosGrilla()
        {
            ...
            try
            {
                ...
                ObjMunDescuento.IdFormularioImpuesto = this.ViewState["IdImpuesto"] != null ? this.ViewState["IdImpuesto"].ToString().Trim() : null;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!(this.Page.IsPostBack))
            {
                ...
                this.ViewState["Tipo"] = Request.QueryString["Tipo"].ToString().Trim();
                //--
                if (this.ViewState["Tipo"].ToString().Trim().Equals("1"))
                {
                    this.LbTitulo.Text = "LISTA DE MUNICIPIOS CON FECHAS VENCIDAS";
                }
                else if (this.ViewState["Tipo"].ToString().Trim().Equals("2"))
                {
                    this.LbTitulo.Text = "LISTA DE MUNICIPIOS CON VENCIMIENTOS DIA ACTUAL";
                }
                else if (this.ViewState["Tipo"].ToString().Trim().Equals("3"))
                {
                    this.LbTitulo.Text = "LISTA DE MUNICIPIOS CON FECHAS POR VENCER";
                }
        ...
        protected void RadGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                this.RadGrid1.DataSource = (DataTable)this.Session["DtCalendarioTrib"];
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "BtnExportarExcel")
                {
                    ...
                    dtExportar = (DataTable)this.Session["DtCalendarioTrib"];
        ...
        protected void ExportarDatosExcel(DataTable DtDatosExportar)
        {
            try
            {
                if (DtDatosExportar.Rows.Count > 0)
                {
                    ...
                    string cNombreFileExcel = "RptCalendario_" + _DiaActual + "_" + _MesActual + ".xlsx";
                    ...
                    for (int ncol = 0; ncol < DtDatosExportar.Columns.Count; ncol++)
                    {
                        ...
                        {
                            #region DETALLE DE LOS REGISTROS EN EL ARCHIVO DE EXCEL
                            //if (ncol == 7)
                            //{
