...
{
    public partial class FrmConfigContabilizacion : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!(this.Page.IsPostBack))
            {
                ...
                this.LbTitulo.Text = "HOJA DE CONFIGURACIÓN DE " + this.ViewState["NombreImpuesto"].ToString().Trim();
            }
            else
            {
                ObjUtils.CambiarGrillaAEspanol(RadGrid1);
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                ...
                {
                    #region INGRESAR A CONFIGURACION DEL IMPUESTO
                    GridDataItem item = (GridDataItem)e.Item;
                    int _IdFormConfiguracion = Convert.ToInt32(item.GetDataKeyValue("idformulario_configuracion").ToString().Trim());
                    string _NumeroRenglon = item["numero_renglon"].Text.ToString().Trim();
                    string _DescripcionRenglon = item["descripcion_renglon"].Text.ToString().Trim();
                    CheckBox ChkRenglonCalculado = item["renglon_calculado"].Controls[0] as CheckBox;
                    CheckBox ChkRenglonContabilizar = item["renglon_contabilizar"].Controls[0] as CheckBox;
                    //--
                    if (ChkRenglonContabilizar.Checked == true)
                    {
                        #region AQUI MOSTRAMOS EL FORMULARIO DE CONFIGURACION
                        ...
                        Ventana.NavigateUrl = "/Controles/Administracion/Clientes/FrmAddConfigRenglonContabilizar.aspx?IdConfImpuesto=" + _IdFormConfiguracion +
                    ...
                    else
                    {
                        ...
                        RadWindow Ventana = new RadWindow();
                        Ventana.Modal = true;
                        string _MsgMensaje = "Señor usuario, No se puede Configurar este renglon ya que esta para no ser Contabilizado. Por favor validar la configuración e intentar nuevamente !";
