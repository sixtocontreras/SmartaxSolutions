...
{
    public partial class CtrlTipoCtrlActividades : System.Web.UI.UserControl
    {
        private static readonly ILog _log = LogManager.GetLogger(FixedData.LOG_AUDITORIA_NAME);
        RadWindow Ventana = new RadWindow();

        TipoCtrlActividad ObjTipoCtrlAct = new TipoCtrlActividad();
        ...
        public DataSet GetDatosGrilla()
        {
            ...
            try
            {
                ObjTipoCtrlAct.TipoConsulta = 1;
                ObjTipoCtrlAct.IdEstado = null;
                ObjTipoCtrlAct.MotorBaseDatos = Session["MotorBaseDatos"].ToString().Trim();

                //Mostrar los formularios
                ObjetoDataTable = ObjTipoCtrlAct.GetAllTipoActividades();
                ObjetoDataTable.PrimaryKey = new DataColumn[] { ObjetoDataTable.Columns["idtipo_ctrl_actividades"] };
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al listar los tipos de actividades. Motivo: " + ex.ToString();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.Page.Title = this.Page.Title + "Tipo Naturaleza";
        ...
        protected void RadGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                ...
                this.RadGrid1.DataMember = "DtTipoCtrlActividad";
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "BtnLogsAuditoria")
                {
                    ...
                    string _ModuloApp = "TIPO_CTRL_ACTIVIDAD";
        ...
        protected void RadGrid1_ItemCreated(object sender, Telerik.Web.UI.GridItemEventArgs e)
        {
            if ((e.Item is GridEditableItem && e.Item.IsInEditMode))
            {
                try
                {
                    ...
                    GridTextBoxColumnEditor editor = (GridTextBoxColumnEditor)item.EditManager.GetColumnEditor("tipo_ctrl_actividad");
        ...
        protected void RadGrid1_InsertCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            DataTable TablaDatos = this.FuenteDatos.Tables["DtTipoCtrlActividad"]; ;
            DataRow NuevaFila = TablaDatos.NewRow();
            TablaDatos.PrimaryKey = new DataColumn[] { TablaDatos.Columns["idtipo_ctrl_actividades"] };
            DataRow[] TodosValores = TablaDatos.Select("", "idtipo_ctrl_actividades", DataViewRowState.CurrentRows); ;
            ...
            try
            {
                ...
                if ((NuevaFila != null))
                {
                    ...
                    ObjTipoCtrlAct.IdTipoCtrlActividades = null;
                    ObjTipoCtrlAct.TipoActividad = NuevaFila["tipo_ctrl_actividad"].ToString().Trim().ToUpper();
                    ObjTipoCtrlAct.IdEstado = NuevaFila["id_estado"].ToString().Trim().ToUpper();
                    ObjTipoCtrlAct.IdUsuario = Int32.Parse(this.Session["IdUsuario"].ToString().Trim());
                    ObjTipoCtrlAct.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                    ObjTipoCtrlAct.TipoProceso = 1;

                    //--AQUI SERIALIZAMOS EL OBJETO CLASE
                    JavaScriptSerializer js = new JavaScriptSerializer();
                    string jsonRequest = js.Serialize(ObjTipoCtrlAct);
                    _log.Warn("REQUEST INSERT TIPO CTRL ACTIVIDAD => " + jsonRequest);

                    int _IdRegistro = 0;
                    string _MsgError = "";
                    if (ObjTipoCtrlAct.AddUpTipoActividad(NuevaFila, ref _IdRegistro, ref _MsgError))
                    {
                        NuevaFila["idtipo_ctrl_actividades"] = _IdRegistro;
                        NuevaFila["tipo_ctrl_actividad"] = NuevaFila["tipo_ctrl_actividad"].ToString().Trim().ToUpper();
                        NuevaFila["codigo_estado"] = CmbEstado.ToString().Trim();
                        NuevaFila["fecha_registro"] = DateTime.Now;
                        this.FuenteDatos.Tables["DtTipoCtrlActividad"].Rows.Add(NuevaFila);
                        ...
                        ObjAuditoria.ModuloApp = "TIPO_CTRL_ACTIVIDAD";
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al registrar el Tipo de Naturaleza. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow" + ObjUtils.GetRandom();
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
        ...
        protected void RadGrid1_UpdateCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            TablaDatos = this.FuenteDatos.Tables["DtTipoCtrlActividad"];
            TablaDatos.PrimaryKey = new DataColumn[] { TablaDatos.Columns["idtipo_ctrl_actividades"] };
            DataRow[] changedRows = TablaDatos.Select("idtipo_ctrl_actividades = " + Int32.Parse(editedItem.OwnerTableView.DataKeyValues[editedItem.ItemIndex]["idtipo_ctrl_actividades"].ToString()));
            int _IdRegistro = Int32.Parse(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["idtipo_ctrl_actividades"].ToString().Trim());
            ...
            try
            {
                ...
                string CmbEstado = (editedItem["id_estado"].Controls[0] as RadComboBox).SelectedItem.Text;
                ObjTipoCtrlAct.IdTipoCtrlActividades = _IdRegistro;
                ObjTipoCtrlAct.TipoActividad = changedRows[0]["tipo_ctrl_actividad"].ToString().Trim().ToUpper();
                ObjTipoCtrlAct.IdEstado = changedRows[0]["id_estado"].ToString().Trim().ToUpper();
                ObjTipoCtrlAct.IdUsuario = Int32.Parse(this.Session["IdUsuario"].ToString().Trim());
                ObjTipoCtrlAct.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                ObjTipoCtrlAct.TipoProceso = 2;

                //--AQUI SERIALIZAMOS EL OBJETO CLASE
                JavaScriptSerializer js = new JavaScriptSerializer();
                string jsonRequest = js.Serialize(ObjTipoCtrlAct);
                _log.Warn("REQUEST UPDATE TIPO CTRL ACTIVIDAD => " + jsonRequest);

                string _MsgError = "";
                if (ObjTipoCtrlAct.AddUpTipoActividad(changedRows[0], ref _IdRegistro, ref _MsgError))
                {
                    changedRows[0]["tipo_ctrl_actividad"] = changedRows[0]["tipo_ctrl_actividad"].ToString().Trim().ToUpper();
                    changedRows[0]["codigo_estado"] = CmbEstado.ToString().Trim().ToUpper();
                    this.FuenteDatos.Tables["DtTipoCtrlActividad"].Rows[0].AcceptChanges();
                    this.FuenteDatos.Tables["DtTipoCtrlActividad"].Rows[0].EndEdit();
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al editar el Tipo de Naturaleza. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow" + ObjUtils.GetRandom();
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
        ...
        protected void RadGrid1_DeleteCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                ObjTipoCtrlAct.IdUsuario = Int32.Parse(this.Session["IdUsuario"].ToString().Trim());
                ObjTipoCtrlAct.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                ObjTipoCtrlAct.TipoProceso = 3;

                //--AQUI SERIALIZAMOS EL OBJETO CLASE
                JavaScriptSerializer js = new JavaScriptSerializer();
                string jsonRequest = js.Serialize(ObjTipoCtrlAct);
                _log.Warn("REQUEST DELETE TIPO CTRL ACTIVIDAD => " + jsonRequest);

                if (ObjTipoCtrlAct.AddUpTipoActividad(changedRows[0], ref _IdRegistro, ref _MsgError))
                {
                    this.FuenteDatos.Tables["DtTipoCtrlActividad"].Rows.Find(_IdRegistro).Delete();
                    this.FuenteDatos.Tables["DtTipoCtrlActividad"].AcceptChanges();
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al eliminar el Tipo de Naturaleza. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow" + ObjUtils.GetRandom();
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
