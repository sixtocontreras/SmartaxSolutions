...
{
    public partial class CtrlTipoCargo : System.Web.UI.UserControl
    {
        private static readonly ILog _log = LogManager.GetLogger(FixedData.LOG_AUDITORIA_NAME);
        RadWindow Ventana = new RadWindow();

        TiposCargo ObjTipoCargo = new TiposCargo();
        ...
        public DataSet GetDatosGrilla()
        {
            ...
            try
            {
                ObjTipoCargo.TipoConsulta = 1;
                ObjTipoCargo.IdEstado = null;
                ObjTipoCargo.MotorBaseDatos = Session["MotorBaseDatos"].ToString().Trim();

                //Mostrar los formularios
                ObjetoDataTable = ObjTipoCargo.GetAllTipoCargo();
                ObjetoDataTable.PrimaryKey = new DataColumn[] { ObjetoDataTable.Columns["idtipo_cargo"] };
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                this.Page.Title = this.Page.Title + "Tipo Cargo";
        ...
        protected void RadGrid1_NeedDataSource(object sender, GridNeedDataSourceEventArgs e)
        {
            try
            {
                ...
                RadGrid1.DataMember = "DtTipoCargo";
        ...
        protected void RadGrid1_ItemCommand(object sender, GridCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "BtnLogsAuditoria")
                {
                    ...
                    string _ModuloApp = "TIPO_ACT_CARGO";
        ...
        protected void RadGrid1_ItemCreated(object sender, Telerik.Web.UI.GridItemEventArgs e)
        {
            if ((e.Item is GridEditableItem && e.Item.IsInEditMode))
            {
                try
                {
                    ...
                    GridTextBoxColumnEditor editor = (GridTextBoxColumnEditor)item.EditManager.GetColumnEditor("tipo_cargo");
                ...
                catch (Exception ex)
                {
                    ...
                    RadWindow Ventana = new RadWindow();
                    Ventana.Modal = true;
                    string _MsgError = "Error con el evento RadGrid1_ItemCreated del Tipo de Tarifa. Motivo: " + ex.ToString();
                    Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                    Ventana.ID = "RadWindow2";
                    Ventana.VisibleOnPageLoad = true;
                    Ventana.Visible = true;
        ...
        protected void RadGrid1_InsertCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            DataTable TablaDatos = this.FuenteDatos.Tables["DtTipoCargo"]; ;
            DataRow NuevaFila = TablaDatos.NewRow();
            TablaDatos.PrimaryKey = new DataColumn[] { TablaDatos.Columns["idtipo_cargo"] };
            DataRow[] TodosValores = TablaDatos.Select("", "idtipo_cargo", DataViewRowState.CurrentRows); ;
            ...
            try
            {
                ...
                if ((NuevaFila != null))
                {
                    ...
                    ObjTipoCargo.IdTipoCargo = null;
                    ObjTipoCargo.TipoCargo = NuevaFila["tipo_cargo"].ToString().Trim();
                    ObjTipoCargo.IdEstado = NuevaFila["id_estado"].ToString().Trim();
                    ObjTipoCargo.IdUsuario = Int32.Parse(this.Session["IdUsuario"].ToString().Trim());
                    ObjTipoCargo.IdEmpresa = Int32.Parse(this.Session["IdEmpresa"].ToString().Trim());
                    ObjTipoCargo.IdRol = Int32.Parse(this.Session["IdRol"].ToString().Trim());
                    ObjTipoCargo.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                    ObjTipoCargo.TipoProceso = 1;

                    //--AQUI SERIALIZAMOS EL OBJETO CLASE
                    JavaScriptSerializer js = new JavaScriptSerializer();
                    string jsonRequest = js.Serialize(ObjTipoCargo);
                    _log.Warn("REQUEST INSERT TIPO CARGO => " + jsonRequest);

                    int _IdRegistro = 0;
                    string _MsgError = "";
                    if (ObjTipoCargo.AddUpTipoCargo(NuevaFila, ref _IdRegistro, ref _MsgError))
                    {
                        NuevaFila["idtipo_cargo"] = _IdRegistro;
                        NuevaFila["tipo_cargo"] = NuevaFila["tipo_cargo"].ToString().Trim().ToUpper();
                        NuevaFila["codigo_estado"] = CmbEstado.ToString().Trim();
                        NuevaFila["fecha_registro"] = DateTime.Now;
                        this.FuenteDatos.Tables["DtTipoCargo"].Rows.Add(NuevaFila);
                        ...
                        ObjAuditoria.ModuloApp = "TIPO_ACT_CARGO";
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al registrar el Tipo de Tarifa. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow2";
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
        ...
        protected void RadGrid1_UpdateCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            TablaDatos = this.FuenteDatos.Tables["DtTipoCargo"];
            TablaDatos.PrimaryKey = new DataColumn[] { TablaDatos.Columns["idtipo_cargo"] };
            DataRow[] changedRows = TablaDatos.Select("idtipo_cargo = " + Int32.Parse(editedItem.OwnerTableView.DataKeyValues[editedItem.ItemIndex]["idtipo_cargo"].ToString()));
            int _IdRegistro = Int32.Parse(e.Item.OwnerTableView.DataKeyValues[e.Item.ItemIndex]["idtipo_cargo"].ToString().Trim());
            ...
            try
            {
                ...
                string CmbEstado = (editedItem["id_estado"].Controls[0] as RadComboBox).SelectedItem.Text;
                ObjTipoCargo.IdTipoCargo = _IdRegistro;
                ObjTipoCargo.IdEmpresa = Int32.Parse(this.Session["IdEmpresa"].ToString().Trim());
                ObjTipoCargo.IdRol = Int32.Parse(this.Session["IdRol"].ToString().Trim());
                ObjTipoCargo.IdUsuario = Int32.Parse(this.Session["IdUsuario"].ToString().Trim());
                ObjTipoCargo.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                ObjTipoCargo.TipoProceso = 2;

                //--AQUI SERIALIZAMOS EL OBJETO CLASE
                JavaScriptSerializer js = new JavaScriptSerializer();
                string jsonRequest = js.Serialize(ObjTipoCargo);
                _log.Warn("REQUEST UPDATE TIPO CARGO => " + jsonRequest);

                string _MsgError = "";
                if (ObjTipoCargo.AddUpTipoCargo(changedRows[0], ref _IdRegistro, ref _MsgError))
                {
                    changedRows[0]["tipo_cargo"] = changedRows[0]["tipo_cargo"].ToString().Trim().ToUpper();
                    changedRows[0]["codigo_estado"] = CmbEstado.ToString().Trim().ToUpper();
                    this.FuenteDatos.Tables["DtTipoCargo"].Rows[0].AcceptChanges();
                    this.FuenteDatos.Tables["DtTipoCargo"].Rows[0].EndEdit();
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al editar el Tipo de Tarifa. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow2";
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
        ...
        protected void RadGrid1_DeleteCommand(object source, Telerik.Web.UI.GridCommandEventArgs e)
        {
            ...
            try
            {
                ...
                ObjTipoCargo.IdRol = Int32.Parse(this.Session["IdRol"].ToString().Trim());
                ObjTipoCargo.MotorBaseDatos = this.Session["MotorBaseDatos"].ToString().Trim();
                ObjTipoCargo.TipoProceso = 3;

                //--AQUI SERIALIZAMOS EL OBJETO CLASE
                JavaScriptSerializer js = new JavaScriptSerializer();
                string jsonRequest = js.Serialize(ObjTipoCargo);
                _log.Warn("REQUEST DELETE TIPO CARGO => " + jsonRequest);

                if (ObjTipoCargo.AddUpTipoCargo(changedRows[0], ref _IdRegistro, ref _MsgError))
                {
                    this.FuenteDatos.Tables["DtTipoCargo"].Rows.Find(_IdRegistro).Delete();
                    this.FuenteDatos.Tables["DtTipoCargo"].AcceptChanges();
            ...
            catch (Exception ex)
            {
                ...
                RadWindow Ventana = new RadWindow();
                Ventana.Modal = true;
                string _MsgError = "Error al eliminar el Tipo de Tarifa. Motivo: " + ex.ToString();
                Ventana.NavigateUrl = "/Controles/General/FrmMensaje.aspx?strMensaje=" + _MsgError;
                Ventana.ID = "RadWindow2";
                Ventana.VisibleOnPageLoad = true;
                Ventana.Visible = true;
